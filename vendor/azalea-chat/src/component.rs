...
impl FormattedText {
    ...
    pub fn from_nbt_compound(compound: simdnbt::borrow::NbtCompound) -> Option<Self> {
        ...
        if let Some(text) = compound.get("text") {
            ...
            // mc-agents: a component this crate cannot resolve used to fail the whole packet, and
            // the message it was in never reached the bot. Keep the message and name what is missing.
            let name = score.string("name").map(|s| s.to_string()).unwrap_or_default();
            component = FormattedText::Text(TextComponent::new(format!("[score {name}]")));
        } else if let Some(selector) = compound.string("selector") {
            component = FormattedText::Text(TextComponent::new(format!("[selector {selector}]")));
        } else if let Some(keybind) = compound.string("keybind") {
            component = FormattedText::Text(TextComponent::new(format!("[{keybind}]")));
        } else if compound.get("object").is_some() {
            component = FormattedText::Text(TextComponent::new("[object]".to_owned()));
        } else if let Some(tag) = compound.get("") {
            return FormattedText::from_nbt_tag(tag);
        } else if compound.get("nbt").is_some() {
            component = FormattedText::Text(TextComponent::new("[nbt]".to_owned()));
        } else if compound.get("sprite").is_some() || compound.get("player").is_some() {
            // 26.x object components carry their kind in these fields rather than under "object".
            component = FormattedText::Text(TextComponent::new("[object]".to_owned()));
        } else {
            return None;
        }
